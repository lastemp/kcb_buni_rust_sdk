...
impl AdditionalDetails {
    pub fn new(
        ...
        if biller_reference_no.is_empty() || biller_reference_no.replace(" ", "").trim().len() == 0
        {
            return Err(String::from("biller reference no is empty"));
        }
        // biller_reference_no has a max length of 50 characters
        else if biller_reference_no.trim().len() >= 2 && biller_reference_no.trim().len() <= 50 {
            // receiver_identifier is valid
        } else {
            return Err(String::from("biller reference no has invalid length"));
        }
    ...
    pub fn get_biller_reference_no(&self) -> String {
        let biller_reference_no = &self.biller_reference_no;
        biller_reference_no.to_string()
    }
